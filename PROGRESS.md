...
**Fresh-start address reset (Tom's call, then made MEMORABLE on his feedback):** all 254 `household_email_addresses` regenerated server-side. Final scheme: **base name + street number** (`burke146@alfred.getchez.com`) — the say-it-to-your-plumber format; households without a property get base + two digits; duplicate finals get a sequence digit; a two-phase update (park on temp values, then assign finals) dodges transitional unique-constraint collisions. Base derivation strips the old generator's artifacts (leading street digits, trailing collision digits, "-xxxx" failsafe suffixes) from the pre-reset local. Tom's household holds `burke146@` (swapped from an orphaned zero-user test household that had claimed it by id order). Old addresses 404 immediately ("Unknown email address" — verified); new addresses route (verified). The app fetches the address dynamically everywhere (dashboard caption, quiz reveal, Share button), so every household simply sees their new address at next login — no client change, no build dependency. Old values snapshotted in `household_email_addresses_backup_20260708` for "what was my old address" recovery; drop the table once the transition settles. Smoke-fixture household's new address: `securitysmoketest52@alfred.getchez.com` (future fixture runs must look it up).
...
**Server (deployed):** classifier gains a conservative standing-arrangement detector (`suggestedRoutines`, explicit evidence only — contract language / stated cadence; max_tokens 1536→2048; prompt-injection preamble covers the new field). analyze-document gains the matching `standing_arrangements` extraction for uploaded service agreements. Both normalize into evidence-only `kind: routine` rows (category, interval_days, cadence_phrase, activeMonths as month numbers, quoted_text, cost, matched contractor_id) on the follow-ups review item, which now fires when EITHER follow-ups or routines exist ("Standing service spotted from X", `review_actions` when routine-only, legacy suggested_tasks never mirrors routine rows). process-inbox-item: routine rows remapped to "just a task" apply through the SAME server task path via `payload_overrides.apply_as = "task"`. **Fixture finding:** signed-agreement confirmations classify as `contractor_quote`, which suppressed the routine lane — suppression relaxed to insurance claims only (the classifier's own accepted-vs-proposed gate is the filter; rows are ask-first).

**iOS (Chez build green):** NEW `RoutineSeeder.createFromIngestion` — the MANDATORY apply wrapper: canonical → `routineKindFor` eligibility (nil = template-backed → card downgrades row to Task destination), serviceKey/kind dedup (kind-level dedup for .otherService is the painter-bug class), evidence-interval → cadence-type banding with `defaultCadenceForRoutineKind` fallback, `weekly_has_days_of_week` synthesis from today's weekday, `cadence_source: email_ingestion`, active-with-vendor / pending_vendor-without + `linkVendorTasksToRoutine`. NEW `Haven/Shared/Components/CompactCadenceEditor.swift` (frequency menu + ActiveMonthsPicker reuse). Card: routine rows get Routine/Task/Chez destinations, the inline cadence editor, and a `ContractorPickerSheet` vendor row; ineligible categories drop the Routine option. ApplyEngine gains the routine applier (created/duplicate/notEligible → applied/duplicate/failed). `InvoiceCadenceCoordinator.apply` routes program-kind system cadences through the same wrapper (dedup makes repeat invoices no-ops); interval + CadenceSuggestionCard path remains for non-program systems. Also fixed an M1 card bug: the chez lane's category decoded from `chez_category` but the wire key is `category`.

**Migration `20270125`:** `routines_cadence_source_check` extended with `email_ingestion` (pushed).

**Verified live (fixture household, restored to baseline):** pool seasonal-agreement email → routine action with interval_days 7 + active_months [5-9] + cost 14500¢ + matched vendor stamped, `review_actions`, no legacy mirror; HVAC "annual tune-up" reminder → NO routine, task auto-added (conservatism holds); constraint-level insert of the exact createFromIngestion shape passes all CHECKs incl. the new source value; routine→task downgrade via `apply_as` creates the reframed task; ledger completion works. Category chain spot-checked: "Pool Service"→"Pool/Spa"→.poolService, "Pet Waste Removal"→"Pet Waste"→.petWaste, all prompt categories resolve. On-device apply test of the routine lane remains for Tom's next TestFlight pass.
