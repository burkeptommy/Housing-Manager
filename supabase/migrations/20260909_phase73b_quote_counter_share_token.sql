-- Phase 73 sub-phase B fix: counter_provider_quote was inserting the
-- cloned counter row without populating `public_share_token`, which is
-- NOT NULL. The parent's token can't be reused (unique index), so the
-- cloned row needs its own. Re-replace the function to gen_random_uuid()
-- the new token at insert time.
