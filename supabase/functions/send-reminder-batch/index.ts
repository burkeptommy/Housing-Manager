...
function reminderCopy(index: number): { subjectSuffix: string; intro: string } {
  switch (index) {
    case 0:
      return {
        subjectSuffix: "is still waiting on Haven",
        intro: "A quick reminder: your invitation is still active.",
      };
    case 1:
      return {
        subjectSuffix: "your Haven invite is still open",
        intro: "Just a friendly nudge — your invitation is still waiting.",
      };
    default:
      return {
        subjectSuffix: "last chance to join Haven",
      ...
      <tr>
        <td align="center" style="padding:48px 16px;">
          <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;background:#F8F6F1;border-radius:16px;border:1px solid #F0EBE1;">
            ...
            <tr><td style="padding:16px 32px 0 32px;font-family:Georgia,serif;font-size:15px;line-height:1.55;color:#1B2A4A;">${escapeHtml(intro)} ${escapeHtml(ctx.inviterName)} invited you to join <strong>${escapeHtml(householdName)}</strong> on Haven.</td></tr>
...
function buildText(ctx: ReminderContext, intro: string): string {
  ...
  const lines = [
    `Hi ${inviteeName},`,
    "",
    `${intro} ${ctx.inviterName} invited you to join ${householdName} on Haven.`,
  ];
