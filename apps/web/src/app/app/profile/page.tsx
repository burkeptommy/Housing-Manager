      ...
      <div className="px-6 pb-6">
        <div className="flex items-end -mt-12 mb-4">
          <div className="relative">
            <div className="w-24 h-24 rounded-full border-4 border-white bg-neutral-200 flex items-center justify-center overflow-hidden shadow-lg">
              {user.avatarUrl ? (
        ...
        <h2 className="text-xl font-bold text-neutral-900">{user.displayName}</h2>
        <p className="text-neutral-500 text-sm mt-0.5">{user.householdName}</p>
        ...
        <div className="grid grid-cols-2 gap-4 mt-6">
          <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl p-4 border border-emerald-100">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-medium text-neutral-600">Contribution Score</span>
            </div>
            <p className="text-3xl font-bold text-emerald-600">{user.contributionScore}</p>
            <p className="text-xs text-neutral-500 mt-1">{user.tasksCompleted} tasks completed</p>
          </div>
          ...
          <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-xl p-4 border border-amber-100">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-medium text-neutral-600">Active Streak</span>
            </div>
            <p className="text-3xl font-bold text-amber-600">{user.weekStreak} weeks</p>
            <p className="text-xs text-neutral-500 mt-1">Keep it going!</p>
          </div>
        </div>
      </div>
    </div>
        ...
        {!isEditing ? (
          ...
          <div className="flex gap-2">
            <button
              ...
              className="inline-flex items-center gap-1 px-3 py-1.5 text-sm font-medium text-neutral-600 hover:bg-neutral-100 rounded-lg transition-colors"
            >
              <X className="w-4 h-4" />
              Cancel
            </button>
          ...
          {isEditing ? (
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 border border-neutral-300 rounded-lg focus:ring-2 focus:ring-emerald-600 focus:border-transparent"
              />
            </div>
          ) : (
      ...
      {showVerifyPhone && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-xl p-6 max-w-sm w-full">
            <h4 className="text-lg font-semibold text-neutral-900 mb-2">Verify Your Phone</h4>
            ...
            <div className="flex gap-2 mb-4">
              {[...Array(6)].map((_, i) => (
                <input
                  ...
                  type="text"
                  maxLength={1}
                  className="w-10 h-12 text-center text-xl font-bold border border-neutral-300 rounded-lg focus:ring-2 focus:ring-emerald-600 focus:border-transparent"
                />
              ))}
            </div>
            <button
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-neutral-500">Control how and when Haven notifies you</p>
      </div>
      ...
      {Object.entries(notifications).map(([category, settings]) => {
        ...
        return (
          <div key={category} className="bg-neutral-50 rounded-xl overflow-hidden">
            <div className="flex items-center gap-3 px-4 py-3 bg-neutral-100">
              <Icon className="w-5 h-5 text-neutral-600" />
              <span className="font-medium text-neutral-700">{categoryLabels[category]}</span>
              {isSystem && (
...
function WalletTab() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-semibold text-neutral-900 mb-1">Digital Wallet</h3>
        <p className="text-sm text-neutral-500">Manage payment methods for reimbursements and split expenses</p>
      </div>

      {/* Linked Accounts */}
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h4 className="font-medium text-neutral-700">Linked Accounts</h4>
          <button
        ...
        {accounts.length > 0 ? (
          <div className="space-y-2">
            {accounts.map((account) => (
              ...
              >
                ...
                <div className="flex-1">
                  <p className="font-medium text-neutral-900 capitalize">{account.type}</p>
          ...
          <div className="text-center py-8 bg-neutral-50 rounded-xl">
            ...
            <p className="text-neutral-500">No accounts linked yet</p>
          </div>
        )}
      </div>
      ...
      <div className="space-y-3">
        <h4 className="font-medium text-neutral-700">Connect a New Account</h4>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {['venmo', 'zelle', 'paypal'].map((type) => (
            <button
              key={type}
              className="flex items-center justify-center gap-2 p-4 bg-white border border-neutral-200 rounded-xl hover:border-emerald-300 hover:bg-emerald-50 transition-colors"
            >
              <span className="text-xl">{accountIcons[type]}</span>
              <span className="font-medium text-neutral-700 capitalize">Connect {type}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="p-4 bg-neutral-50 rounded-xl">
        <div className="flex items-center gap-3 mb-3">
          ...
          <h4 className="font-medium text-neutral-700">Default Payment Method</h4>
        </div>
        ...
        <div className="flex items-center gap-3 p-3 bg-white rounded-lg border border-neutral-200">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-neutral-500">@bobchen</p>
          </div>
...
function SecurityTab({ user }: { user: typeof mockUserData }) {
  ...
  return (
    <div className="space-y-8">
      {/* Social Privacy */}
      <div>
        <h3 className="text-lg font-semibold text-neutral-900 mb-1">Social Privacy</h3>
        <p className="text-sm text-neutral-500 mb-4">Control who can see your project posts</p>
        ...
        <div className="space-y-2">
          {[
            ...
            >
              <option.icon className={`w-5 h-5 ${visibility === option.value ? 'text-emerald-600' : 'text-neutral-400'}`} />
              <div className="flex-1 text-left">
                <p className={`font-medium ${visibility === option.value ? 'text-emerald-900' : 'text-neutral-900'}`}>
                  {option.label}
                </p>
      ...
      <div>
        ...
        <div className="space-y-3">
          ...
          <button
            ...
            className="w-full flex items-center gap-4 p-4 bg-white rounded-xl border border-neutral-200 hover:bg-neutral-50 transition-colors"
          >
        ...
        <div className="space-y-2">
          {sessions.map((session) => {
            ...
            return (
              ...
              >
                <div className="p-2 bg-neutral-100 rounded-lg">
                  <DeviceIcon className="w-5 h-5 text-neutral-600" />
                </div>
                <div className="flex-1">
      ...
      <div className="border-t border-neutral-200 pt-8">
        ...
        <div className="space-y-3">
          {isAdmin && (
            <button className="w-full flex items-center gap-4 p-4 bg-white rounded-xl border border-neutral-200 hover:bg-amber-50 hover:border-amber-300 transition-colors">
              <div className="p-2 bg-amber-100 rounded-lg">
                <Crown className="w-5 h-5 text-amber-600" />
              </div>
              <div className="flex-1 text-left">
                <p className="font-medium text-neutral-900">Transfer Admin Rights</p>
                <p className="text-sm text-neutral-500">Give Head of Household role to another adult</p>
              </div>
              <ChevronRight className="w-5 h-5 text-neutral-400" />
            </button>
          )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-xl p-6 max-w-sm w-full">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h4 className="text-lg font-semibold text-neutral-900">Leave Household?</h4>
            </div>
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-neutral-200 overflow-hidden">
      ...
      >
        ...
        <span className="flex-1 font-medium text-neutral-900">{title}</span>
        <ChevronDown
...
export default function ProfilePage() {
  ...
  return (
    <div className="pb-32 lg:pb-8">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl lg:text-3xl font-bold text-neutral-900">My Profile</h1>
        <p className="text-neutral-500 mt-1">Manage your personal settings and preferences</p>
      </div>
