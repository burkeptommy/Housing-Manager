...
function ProjectCard({
  ...
  return (
    <div
      onClick={onClick}
      className="group bg-white rounded-xl shadow-sm border border-neutral-200 overflow-hidden cursor-pointer hover:shadow-md hover:border-emerald-300 transition-all"
    >
      {/* Cover Image */}
      <div className="relative h-48 overflow-hidden">
        ...
        <div className="absolute top-3 left-3 right-3 flex items-start justify-between">
          <span className="px-2 py-1 bg-white/90 backdrop-blur-sm rounded-md text-xs font-medium text-neutral-700">
            {project.category}
          </span>
          {!isWishlist && (
        ...
        {!isWishlist && (
          <div className="absolute bottom-3 left-3 right-3 flex items-center gap-2">
            {daysUntil !== null && (
              <span className="px-2 py-1 bg-white/90 backdrop-blur-sm rounded-md text-xs font-medium text-neutral-700 flex items-center gap-1">
      ...
      <div className="p-4">
        ...
        {!isWishlist && (
          <div className="flex items-center justify-between mt-4 pt-4 border-t border-neutral-100">
            <div>
              ...
              <p className="font-semibold text-neutral-900">{formatCurrency(project.estimatedBudget)}</p>
            </div>
        ...
        {isWishlist && (
          <div className="mt-4 pt-4 border-t border-neutral-100">
            <p className="text-xs text-neutral-500">Estimated Budget</p>
            <p className="font-semibold text-neutral-900">{formatCurrency(project.estimatedBudget)}</p>
          </div>
        )}
      </div>
  ...
  return (
    <div className="columns-2 md:columns-3 lg:columns-4 gap-4 space-y-4">
      {images.map((image) => (
        <div
          key={image.id}
          className="break-inside-avoid relative rounded-xl overflow-hidden group cursor-pointer bg-neutral-100"
        >
          <Image
            src={image.url}
            alt=""
      ...
      <div
        onClick={onAddImage}
        className="break-inside-avoid flex flex-col items-center justify-center p-8 rounded-xl border-2 border-dashed border-neutral-300 hover:border-emerald-500 hover:bg-emerald-50 cursor-pointer transition-colors aspect-square"
      >
        <ImagePlus className="w-8 h-8 text-neutral-400" />
        <p className="text-sm text-neutral-500 mt-2">Add Inspiration</p>
      </div>
    </div>
  );
}
      ...
      <div className="flex flex-wrap gap-2">
        {items.map((item) => (
          <button
            key={item}
            onClick={() => onAdd(item)}
            className="inline-flex items-center gap-1 px-3 py-1.5 bg-white rounded-full text-sm font-medium text-neutral-700 border border-amber-200 hover:border-emerald-400 hover:bg-emerald-50 transition-colors"
          >
...
function TaskCard({
  ...
  return (
    <div
      ...
      className={`bg-white rounded-lg border border-neutral-200 p-3 shadow-sm hover:shadow-md transition-all cursor-grab active:cursor-grabbing ${
        isDragging ? 'opacity-50 scale-95' : ''
      }`}
    >
      <div className="flex items-start gap-2">
        <GripVertical className="w-4 h-4 text-neutral-300 mt-1" />
        <div className="flex-1 min-w-0">
          ...
          {task.recommendedVendor && (
            <div className="mt-3 p-2 bg-neutral-50 rounded-lg">
              <p className="text-xs text-neutral-500 mb-1.5">Recommended Pro</p>
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-full bg-emerald-100 flex items-center justify-center">
                  <Wrench className="w-4 h-4 text-emerald-600" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-neutral-900 truncate">{task.recommendedVendor.name}</p>
                  <div className="flex items-center gap-1">
                    <Star className="w-3 h-3 text-amber-400 fill-amber-400" />
                    <span className="text-xs text-neutral-600">{task.recommendedVendor.rating}</span>
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="font-semibold text-neutral-900">Budget Overview</h3>
          <p className="text-sm text-neutral-500">Estimated vs Actual Spend</p>
        </div>
        <div className="text-right">
          <p className="text-2xl font-bold text-neutral-900">{formatCurrency(actual)}</p>
          <p className="text-sm text-neutral-500">of {formatCurrency(estimated)}</p>
        </div>
      </div>

      {/* Progress Bar */}
      <div className="relative h-8 bg-neutral-100 rounded-lg overflow-hidden mb-4">
        <div
      ...
      <div className="flex items-center justify-between text-sm">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-neutral-600">0-80% Healthy</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded bg-amber-500" />
            <span className="text-neutral-600">81-99% Caution</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded bg-red-500" />
            <span className="text-neutral-600">100%+ Over</span>
          </div>
        </div>
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-neutral-200 overflow-hidden">
      <div className="px-6 py-4 border-b border-neutral-200">
        <h3 className="font-semibold text-neutral-900">Expense Breakdown</h3>
      </div>
      ...
      {Object.entries(groupedExpenses).map(([category, items]) => {
        ...
        return (
          <div key={category} className="border-b border-neutral-100 last:border-0">
            <div className="px-6 py-3 bg-neutral-50 flex items-center gap-2">
              <Icon className="w-4 h-4 text-neutral-500" />
              <span className="font-medium text-neutral-700">{label}</span>
              <span className="ml-auto font-semibold text-neutral-900">{formatCurrency(categoryTotal)}</span>
            </div>
            <div className="divide-y divide-neutral-100">
              {items.map((expense) => (
                <div key={expense.id} className="px-6 py-3 flex items-center">
                  <div className="flex-1">
                    <p className="text-sm text-neutral-900">{expense.description}</p>
                    {expense.vendor && (
                      <p className="text-xs text-neutral-500">{expense.vendor}</p>
                    )}
                  </div>
                  <div className="text-right">
            ...
            <div className="flex items-center gap-4 mb-6">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 bg-white border border-neutral-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-600 focus:border-transparent"
                />
              </div>
  ...
  return (
    <div className="pb-32 lg:pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-neutral-900">Project Planning</h1>
          <p className="text-neutral-500 mt-1">Dream, plan, and build your home improvements</p>
        </div>
        <div className="flex items-center gap-3">
      ...
      {viewMode === 'active' && activeProjects.length > 0 && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-4">
            ...
            <p className="text-2xl font-bold text-neutral-900">{activeProjects.length}</p>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-4">
            <div className="flex items-center gap-2 text-neutral-500 mb-1">
      ...
      {viewMode === 'wishlist' && (
        <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-6 mb-8 border border-purple-100">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h2 className="text-lg font-semibold text-neutral-900">Your Dream Board</h2>
      ...
      {displayedProjects.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <Link
            href="/app/projects/new"
            className="flex flex-col items-center justify-center p-8 rounded-xl border-2 border-dashed border-neutral-300 hover:border-emerald-500 hover:bg-emerald-50 cursor-pointer transition-colors min-h-[300px]"
          >
            <div className="p-4 bg-neutral-100 rounded-full mb-4">
              <Plus className="w-8 h-8 text-neutral-400" />
            </div>
            <p className="font-medium text-neutral-600">Add New {viewMode === 'active' ? 'Project' : 'Idea'}</p>
