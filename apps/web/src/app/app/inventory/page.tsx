...
function getStatusColor(status: ShoppingStatus) {
  switch (status) {
    case 'NEEDED': return 'bg-slate-100 text-slate-700';
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="p-4 pb-2">
        <h2 className="text-lg font-semibold text-slate-900 mb-3">What do you need?</h2>
      ...
      <div className="px-4 pb-4">
        <p className="text-xs text-slate-500 mb-2">Tip: Separate with commas or say &quot;and&quot;</p>
        <div className="flex flex-wrap gap-2">
          {QUICK_PICKS.map((pick) => (
            <button
              key={pick}
              onClick={() => handleQuickPick(pick)}
              className="px-3 py-1.5 bg-slate-50 text-slate-600 rounded-full text-sm hover:bg-slate-100 transition-colors"
              disabled={isSubmitting}
            >
              + {pick}
            </button>
  ...
  return (
    ...
    >
      <div className={`p-2 rounded-lg ${isActive ? 'bg-emerald-100' : 'bg-slate-100'}`}>
        <ListIcon className={`w-5 h-5 ${isActive ? 'text-emerald-600' : 'text-slate-600'}`} />
      </div>
      <div className="flex-1 min-w-0">
        <div className="font-medium text-slate-900">{list.name}</div>
        <div className="text-sm text-slate-500">{list.itemCount} items</div>
      </div>
      <div className="text-right">
        <div className="text-lg">{getStatusEmoji()}</div>
        <div className="text-xs text-slate-500">{list.statusMessage}</div>
      </div>
    </button>
  );
}
  ...
  return (
    <div className={`flex items-center gap-3 p-3 rounded-lg ${item.isLow ? 'bg-amber-50' : 'bg-white'}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-slate-500">{item.currentLevel} {item.unit} remaining</div>
      </div>
...
function AutoReorderCard({
  ...
  return (
    <div className={`flex items-center gap-3 p-4 rounded-xl border ${item.enabled ? 'bg-white border-slate-200' : 'bg-slate-50 border-slate-100'}`}>
      <div className={`p-2 rounded-lg ${catConfig.color}`}>
        <CatIcon className="w-4 h-4" />
      </div>
      <div className="flex-1 min-w-0">
        <div className={`font-medium ${item.enabled ? 'text-slate-900' : 'text-slate-500'}`}>{item.name}</div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-semibold text-slate-900 text-lg">{list.name}</h2>
      ...
      {Object.entries(groupedItems).map(([category, categoryItems]) => {
        ...
        return (
          <div key={category} className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className={`flex items-center gap-2 px-4 py-2 ${catConfig.color} border-b border-slate-100`}>
              <CatIcon className="w-4 h-4" />
              ...
              {categoryItems.map(item => (
                <div key={item.id} className="flex items-center gap-3 px-4 py-3">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center ${
                    item.status === 'DELIVERED' ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-400'
                  }`}>
                  ...
                  {item.cost && (
                    <div className="text-sm font-medium text-slate-700">${item.cost.toFixed(2)}</div>
                  )}
                </div>
              ))}
            </div>
...
export default function InventoryPage() {
  ...
  return (
    <div className="pb-32 lg:pb-8 max-w-4xl mx-auto">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl lg:text-3xl font-bold text-slate-900">Shopping</h1>
        <p className="text-slate-500 mt-1">Add items and {HOUSING_MANAGER.name} handles procurement</p>
      </div>
        ...
        <>
          ...
          <div className="relative mb-4">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent bg-white"
            />
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className="p-4 border-b border-slate-200 flex items-center justify-between">
              ...
              <span className="text-sm text-slate-500">{inventory.length} items tracked</span>
            </div>
            <div className="divide-y divide-slate-100">
      ...
      {viewMode === 'auto-reorder' && (
        <>
          ...
          <button className="w-full mt-4 flex items-center justify-center gap-2 p-4 border-2 border-dashed border-slate-300 rounded-xl text-slate-600 hover:border-emerald-500 hover:text-emerald-600 transition-colors">
