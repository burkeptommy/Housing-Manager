...
const navigation = [
  ...
  },
  {
    ...
    href: '/internal/project-pipeline',
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function InternalLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!isLoading && !isAuthenticated) {
      ...
      // Non-staff redirect to regular app
      router.push('/app');
    }
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      ...
      <aside className="hidden lg:flex lg:flex-col lg:w-64 lg:fixed lg:inset-y-0 bg-slate-900 dark:bg-slate-950">
        ...
        <div className="flex items-center h-16 px-6 border-b border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="ml-2 text-xs font-medium text-indigo-400 bg-indigo-900/50 px-2 py-0.5 rounded">Internal</span>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 px-4 py-4 space-y-1 overflow-y-auto">
          {navigation.map((item) => {
            const isActive = pathname === item.href || (item.href !== '/internal' && pathname.startsWith(item.href));
            return (
              <Link
                key={item.name}
                href={item.href}
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 flex items-center justify-between h-16 px-4 bg-slate-900 border-b border-slate-800">
        <div className="flex items-center gap-2">
          ...
          <span className="text-lg font-semibold text-white">Haven Internal</span>
        </div>
      </div>
