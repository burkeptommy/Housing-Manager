...
**36 questions across 6 sections.** Quiz state persists to `properties.house_quiz_state` JSONB after every answer. `firstUnresolvedIndex()` resumes at the next unanswered question on reopen. Section milestones (`milestoneIndices = [5, 10, 18, 23, 29, 35]`) trigger fun-fact cards between sections.

**Question kinds (`HouseQuizQuestionKind`):** `singleChoice`, `multiSelect`, `currency`, `yesNoLender`, `vehicleCount`, `vehicleAdd`, `providerSearch`, `caretakers`, **`generatorAdd`** (Phase 19i — Q22 progressive 3-step generator type/fuel/provider form), **`householdContractors`** (Phase 19m — Q15b multi-select contractor chips with inline pickers).
