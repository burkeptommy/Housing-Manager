...
**PropertyDetailView Maintenance tab (Phase 50/Build 89):** Section order: overdue banner > **upcoming vendor visits** (full-width `VendorVisitCard`s for the next 6 vendor tasks on this property) > **your tasks** (DIY only, filtered by `routing == .diyDefault` plus the user's `diyOptedCategories` opt-in for `.diyCapable`; empty state shows green checkmark + "You're all caught up") > **vendor follow-ups** (amber-tinted, only when notes start with "Vendor follow-up:") > home systems > seasonal overview > view-full-schedule footer link > service history. New `PropertyDetailViewModel` computed properties: `vendorPreferenceTier`, `diyTasksForMaintenanceTab`, `vendorVisitTasks`, `vendorFollowUpTasks`, `contractor(for:)`, `recentServiceCost(systemId:contractorId:)`.
