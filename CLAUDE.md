...
**Missing-system auto-create (Phase 54A):** `HouseQuizAnswerMapper.ensureAutoCreatedSystems(propertyId:householdId:db:)` static helper inserts the `home_systems` rows Vendor Coverage knows about but the quiz never directly asked for — `Handyman` and `Mosquito & Tick` always; `Pet Waste` when `properties.attributes["has_pets"] == "true"`; `Chimney` when any `home_systems` row has category `Fireplace` (subtype inferred as wood/gas from the system name); `Snow Removal` when `property.state` falls in the snow-state set (MA, CT, RI, NY, NH, VT, ME, NJ, PA, OH, MI, WI, MN, IA, IL, IN, CO, UT, WY, ID, MT, ND, SD, NE, AK). Wired into `HouseQuizViewModel` inside the existing detached completion Task so new rows land before the final `reconcileAll` pass. `AppState.backfillMissingSystemsOnceIfNeeded()` runs the same rules on every property on auth resolution for existing users; gated on `hasRunMissingSystemBackfillP54A_v1`. New `SystemCategoryRegistry.metaForCategory(_:)` case-insensitive lookup.
