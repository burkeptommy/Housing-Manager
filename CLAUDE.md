...
**DIY vs Vendor Preference (Build 87):** Stored as `properties.attributes["vendor_preference_level"]` (Int 1-10, default 5). Captured at the end of the House Quiz via Q36 (the new `.slider` question kind) and editable later via `Settings → Maintenance Preferences` (`MaintenancePreferencesView`). Both surfaces use the shared `VendorPreferenceSlider` component. `MaintenanceTaskReconciler.resolveAssignment(template:preferenceLevel:)` is the deterministic threshold function: `effort > max(0, (11 - preferenceLevel) * 30) ? .vendor : .personal`. Slider 1 → threshold 300 (DIY everything), 5 → 180 (middle ground), 10 → 0 (vendor everything). The per-system `reconcile` reads the attribute on every pass and uses `resolveAssignment` to decide the assignment for `.either` templates at create time. `reconcileAllForHousehold(householdId:)` is the slider-commit entry point: walks every property in the household, runs `reconcileAll`, then runs the new `flipEitherTasksForPreference` per-property pass that updates EXISTING `.either` rows in place. `flipEitherTasksForPreference` strictly preserves user-touched tasks (notes / completion / reassigned) and tasks with explicit contractor links. Flipping personal→vendor reframes via Phase 19l's "Schedule X: Y" voice (or "Find a contractor for: Y" when no matching contractor exists). Flipping vendor→personal restores the original template title.
...
**Q36 DIY vs Vendor slider (Build 87):** New `.slider` question kind appended to section 6. Captures a 1-10 integer in `HouseQuizAnswer.sliderValue` (resilient decode) and the answer mapper persists to `properties.attributes.vendor_preference_level`, then triggers `MaintenanceTaskReconciler.reconcileAllForHousehold` so existing `.either`-tagged tasks flip based on the threshold function. The shared `VendorPreferenceSlider` component (`Haven/Shared/Components/VendorPreferenceSlider.swift`) is reused by both the quiz and the new `MaintenancePreferencesView` in Settings — Georgia value readout, navy slider, edge labels, and an optional live preview that counts the user's actual either tasks against the current threshold. See "DIY vs Vendor Preference" section under Maintenance for the threshold math and reconciler integration.
