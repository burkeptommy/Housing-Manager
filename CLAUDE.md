...
`HavenCard`, `HavenButton`, `HavenTextField`, `EmptyStateView`, `ErrorView`, `LoadingView`, `ProcessingBanner`, `FamilyAvatarView`, `AlfredLogoView`, `AddressAutocompleteField`, `UnifiedTaskCard`, `VendorVisitCard` (Phase 50 — vendor service-visit card with `.compact` and `.full` styles, used by Dashboard's `VendorScheduleStrip` and PropertyDetailView's vendor visits/follow-ups sections)
